...
.PHONY: build-local
build-local: ## Build binary locally to bin/<OS>/disco.
	@mkdir -p bin/$(OS)
	go build -mod=readonly -ldflags "-s -w $(GO_LDFLAGS)" -o bin/$(OS)/disco main.go
...
build/disco: FORCE generate
	env $(GO_BUILDENV) go build $(GO_BUILDFLAGS) -ldflags '-s -w $(GO_LDFLAGS)' -o build/disco ./
...
install: FORCE build/disco
	install -d -m 0755 "$(DESTDIR)$(PREFIX)/bin"
	install -m 0755 build/disco "$(DESTDIR)$(PREFIX)/bin/disco"
...
goimports: FORCE install-goimports
	@printf "\e[1;36m>> goimports -w -local https://github.com/sapcc/disco\e[0m\n"
	@goimports -w -local github.com/sapcc/disco $(patsubst $(shell awk '$$1 == "module" {print $$2}' go.mod)%,.%/*.go,$(shell go list ./...))
...
help: FORCE
	...
	@printf "  \e[36mbuild/disco\e[0m                  Build disco.\n"
